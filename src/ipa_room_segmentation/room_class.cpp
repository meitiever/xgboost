...
void Room::mergeRoom(Room& room_to_merge, double map_resolution)
{
  ...
  std::vector<int> neighbor_room_ids_clone;
  neighbor_room_ids_clone = neighbor_room_ids_;
  neighbor_room_ids_.clear();
  for (auto it = neighbor_room_ids_clone.begin(); it != neighbor_room_ids_clone.end(); ++it)
  {
    if (*it == id_number_ || *it == room_to_merge.getID()) {
      neighbor_room_ids_.push_back(*it);
    }
  }
