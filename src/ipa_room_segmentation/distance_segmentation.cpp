...
void DistanceSegmentation::segmentMap(const cv::Mat& map_to_be_labeled, cv::Mat& segmented_map,
  double map_resolution_from_subscription, double room_area_factor_lower_limit, double room_area_factor_upper_limit, bool draw_obstacles)
{
  ...
  for (int current_contour = 0; current_contour < saved_contours.size(); current_contour++)
  {
    ...
    do
    {
      ...
      {
        cv::drawContours(segmented_map, saved_contours, current_contour, fill_colour, cv::FILLED);
